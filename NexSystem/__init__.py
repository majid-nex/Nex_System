...
if ENV:
    ...
    RAW_SIBYL = os.environ.get("NEX", "")
    RAW_ENFORCERS = os.environ.get("ENFORCERS", "")
    NEX = list(int(x) for x in os.environ.get("NEX", "").split())
    ...
    Nex_logs = int(os.environ.get("Nex_logs"))
    Nex_approved_logs = int(os.environ.get("Nex_Approved_Logs"))
    ...
    with open(os.path.join(os.getcwd(), "NexSystem\\elevated_users.json"), "r") as f:
        data = json.load(f)
    NEX = data["NEX"]
    ENFORCERS = data["ENFORCERS"]
    INSPECTORS = data["INSPECTORS"]
    Nex_logs = Config.Nex_logs
    Nex_approved_logs = Config.Nex_approved_logs
    GBAN_MSG_LOGS = Config.GBAN_MSG_LOGS
    BOT_TOKEN = Config.BOT_TOKEN

INSPECTORS.extend(Nex)
...
from .client_class import NexClient

try:
    System = NexClient(StringSession(STRING_SESSION), API_ID_KEY, API_HASH_KEY)
except:
    print(traceback.format_exc())
    exit(1)

collection = MONGO_CLIENT["Nex"]["Main"]
